...
nh_(nh), mu_(mu)
{
  for (auto & pair : mu_)
  {
    auto msg = avatar_msgs::IdleControl::Request();
    msg.mode = avatar_msgs::IdleControl::Request::TASK_SPACE;
...
void IdleControlServer::computeArm(ros::Time time, FrankaModelUpdater &arm, avatar_msgs::IdleControl::Request &param)
{
  ...
  switch (param.mode)
  {
    ...
    case avatar_msgs::IdleControl::Request::DISABLED:
    {
      arm.setTorque(Eigen::Vector7d::Zero(), true);
      arm.idle_controlled_ = false;
      break;
    }
    case avatar_msgs::IdleControl::Request::JOINT_SPACE:
    {
      
      // arm.setTorque(~~~, true); // <<- idle_control = true
      break;
    }
    case avatar_msgs::IdleControl::Request::TASK_SPACE:
    {
    ...
    case avatar_msgs::IdleControl::Request::TASK_SPACE_WITH_NULL:
    {
      // default gain (gains are not set)
      if (param.p_gain <= 0.01)
      {
    ...
    case avatar_msgs::IdleControl::Request::TASK_SPACE_DEV_TEST:
    {
      auto fstar = PegInHole::keepCurrentPose(arm.initial_transform_, arm.transform_,arm.xd_,
...
bool IdleControlServer::setTarget(avatar_msgs::IdleControl::Request  &req,
